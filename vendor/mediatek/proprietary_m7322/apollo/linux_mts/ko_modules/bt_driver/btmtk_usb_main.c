...
void btmtk_usb_toggle_rst_pin(void)
{
	...
	do {
		...
		if (node) {
			if (of_property_read_u32(node, "wifi-ctl-gpio", &wifi_ctl_gpio) != 0) {
				BTUSB_INFO("%s, failed to obtain wifi control gpio\n", __func__);
				break;
			}

			if (pLow && pHigh) {
				BTUSB_INFO("%s: Invoke %s(%d)", __func__, set_low_name, wifi_ctl_gpio);
				pLow(wifi_ctl_gpio);
				mdelay(RESET_PIN_SET_LOW_TIME);
				BTUSB_INFO("%s: Invoke %s(%d)", __func__, set_high_name, wifi_ctl_gpio);
				pHigh(wifi_ctl_gpio);
			} else
...
int btmtk_usb_meta_send_data(const u8 *buffer, const unsigned int length)
{
	...
	if (ret < 0) {
		...
		if (ret == COMMAND_TIMED_OUT_ERROR_CODE) {
			err = btmtk_usb_send_assert_cmd_bulk();
			if (err < 0) {
				BTUSB_ERR("%s: send assert cmd fail, tigger hw reset only", __func__);
				btmtk_usb_toggle_rst_pin();
				return err;
		...
		size_t count, loff_t *f_pos)
{
	...
	if (skb_queue_len(&g_data->isoc_in_queue) > 0) {
		...
		while (retval > 0) {
			...
			if (ret_len) {
				BTUSB_ERR("copy to user fail, copy = %zd, ret_len = %ld, count = %zd",
							ret_len, copy, count);
			}

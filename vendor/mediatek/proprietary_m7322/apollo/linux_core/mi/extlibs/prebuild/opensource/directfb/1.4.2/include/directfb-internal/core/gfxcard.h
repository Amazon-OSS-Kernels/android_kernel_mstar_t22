...
typedef struct _GraphicsDeviceFuncs {
      ...
      */
     void (*FlushTextureCache)( void *driver_data, void *device_data,unsigned int u4_addr, unsigned int u4_size );

     /*
      * After the video memory has been written to by the accelerator
      * make sure the CPU won't read back cached data.
      */
     void (*FlushReadCache)( void *driver_data, void *device_data,unsigned int u4_addr, unsigned int u4_size );
     ...
     bool (*DrawLine)      ( void *driver_data, void *device_data,
                             DFBRegion *line,unsigned int width);

...
#define DFB_MSTARKEYPAD_SET_CONFIG   13
...
#define DFB_DEV_IOC_SET_KEYPAD_CONFIG                   _IOW(MDRV_DFB_IOC_MAGIC,   DFB_MSTARKEYPAD_SET_CONFIG, void*)
...
typedef unsigned int DFBWindowDeviceID;
...
typedef enum {
     ...
     DSDESC_PREALLOCATED_IN_VIDEO_64BIT_ADDR = 0x00008000,  /* the preallcate address extend to 64 bit*/

	 DSDESC_ALL          = 0x0000FFFF   /* all of these */
...
typedef enum {
     ...
     DSCAPS_FROM_FBM      = 0x02000000, /* Surface allocated from FBM area. */
     DSCAPS_FROM_JPEG_VDP = 0x04000000, /* Surface allocated from VDP display area. */     
     DSCAPS_3D_CROP_OFFSET= 0x08000000,           
     DSCAPS_FROM_3DMM     = 0x10000000,
     DSCAPS_FOUCE_VIDEOONLY = 0x20000000,
     DSCAPS_FROM_CRSR       = 0x40000000,
     DSCAPS_FROM_MPEG      = 0x80000000,
     DSCAPS_ALL          = 0xFF1317F7,  /* All of these. */
...
typedef enum {
     DDDT_LAYER_FLIP,
     DDDT_DISPLAYLAYER_WAITFORSYNC,
     DDDT_DISPLAYLAYER_SETCONFIGURATION,
     DDDT_DISPLAYLAYER_WARPCURSOR,
     DDDT_WINDOW_FLIP,
     DDDT_SURFACE_FLIP,
     DDDT_SURFACE_LOCK,
     DDDT_SURFACE_RELEASE,
     DDDT_SURFACE_WRITE,
     DDDT_SURFACE_READ,
     DDDT_SURFACE_CLEAR,
     DDDT_SURFACE_FILLRECT,
     DDDT_SURFACE_DRAWLINE,
     DDDT_SURFACE_DRAWLINES,
     DDDT_SURFACE_DRAWRECTANGLE,
     DDDT_SURFACE_FILLTRIANGLE,
     DDDT_SURFACE_FILLTRAPEZOIDS,
     DDDT_SURFACE_FILLRECTANGLES,
     DDDT_SURFACE_FILLSPANS,
     DDDT_SURFACE_FILLTRIANGLES,
     DDDT_SURFACE_BLIT,
     DDDT_SURFACE_TILEBLIT,
     DDDT_SURFACE_BATCHBLIT,
     DDDT_SURFACE_BATCHBLIT2,
     DDDT_SURFACE_STRETCHBLIT,
     DDDT_SURFACE_TEXTURETRIANGLES,
     DDDT_SURFACE_DRAWSTRING,
     DDDT_SURFACE_DRAWGLYPH,
     DDDT_SURFACE_DUMP_BUFFER,
     DDDT_SURFACE_RELEASESOURCE,
     DDDT_SURFACE_FLIPSTEREO,
     DDDT_SURFACE_DRAWMONOGLYPHS,
     DDDT_CARD_FILLRECT,
     DDDT_CARD_DRAWRECT,
     DDDT_CARD_DRAWLINES,
     DDDT_CARD_FILLSPANS,
     DDDT_CARD_FILLTRIGANGLES,
     DDDT_CARD_FILLTRAPEZOIDS,
     DDDT_CARD_FILLQUADRANGLES,
     DDDT_CARD_MONO_GLYPHS,
     DDDT_CARD_BLIT,
     DDDT_CARD_BBLIT,
     DDDT_CARD_BBLIT2,
     DDDT_CARD_TILEBLIT,
     DDDT_CARD_SBLIT,
     DDDT_CARD_TEXTURE_TRIANGLES,
     DDDT_ISAWMANMANAGER_SETWINDOWCONFIG,
     DDDT_ISAWMANMANAGER_LOCK,
     DDDT_IMAGEPROVIDER_GIF_RENDERTO,
     DDDT_IMAGEPROVIDER_JPEG_RENDERTO,
     DDDT_IMAGEPROVIDER_PNG_RENDERTO,
     DDDT_DFB_INIT,
     DDDT_DFB_CREATE,
     DDDT_EMMT_CMD,
     DDDT_IMGRZ,
...
typedef enum {
	 ...
	 DIDTF_MSTARIR		 = 0x00000020, /*Is a IR input device*/
...
/*
 * keypad configuration flags
 */
typedef enum {
     SET_KEYPAD_NONE                        = 0x00,
     SET_KEYPAD_SarChID                     = 0x01,
     SET_KEYPAD_UpBnd                       = 0x02,
     SET_KEYPAD_LoBnd                       = 0x04,
     SET_KEYPAD_KeyLevelNum                 = 0x08,
     SET_KEYPAD_KeyThreshold                = 0x10,
     SET_KEYPAD_KeyCode                     = 0x20,
     SET_KEYPAD_ALL                         = 0x3f
} DFBKeypadConfigFlags;

/*
*  Keypad set Reg Config
*/
typedef struct {
     DFBKeypadConfigFlags Flags;
     unsigned char SarChID;          /// SAR channel ID: from 0
     unsigned char UpBnd;            ///upper bound
     unsigned char LoBnd;            ///low bound
     unsigned char KeyLevelNum;      /// SAR key level number
     unsigned char KeyCode[8];
     unsigned char KeyThreshold[8];  /// SAR key thresholds
}DFBKeypadRegCfg;
...
#define DIDID_KEYPAD        0x0000    /* primary keyboard       */
...
#define DIDID_VIRTUAL            0x0010   /* primary mstar Virtual control */
#define DIDID_KEYBOARD        0x0011    /* primary keyboard       */
#define DIDID_ANY             0x001F    /* no primary device      */
...
typedef enum {
     ...
     DLCONF_3D_CROP_OFFSET   = 0x00000100, 
...
typedef enum {
     ...
     DSBF_CSRC_ASRCxACONST_EXT              =12  /* cf: 1           af: constant alpha */
...
/*
 * ROP2 Options Code for blitting operations.
 */
typedef enum
{
    DFB_ROP2_OP_ZERO ,            // 0
    DFB_ROP2_NOT_PS_OR_PD,        // ~( rop_src | rop_dst );
    DFB_ROP2_NS_AND_PD,           //((~rop_src) & rop_dst);
    DFB_ROP2_NS,                  //~(rop_src);
    DFB_ROP2_PS_AND_ND,           //(rop_src & (~rop_dst));
    DFB_ROP2_ND,                  //~(rop_dst);
    DFB_ROP2_PS_XOR_PD,           //( rop_src ^ rop_dst);
    DFB_ROP2_NOT_PS_AND_PD,       //~(rop_src & rop_dst);
    DFB_ROP2_PS_AND_PD,           //(rop_src & rop_dst);
    DFB_ROP2_NOT_PS_XOR_PD,       //~(rop_dst ^ rop_src);
    DFB_ROP2_PD,                  //rop_dst;
    DFB_ROP2_NS_OR_PD,            //(rop_dst | (~rop_src));
    DFB_ROP2_PS,                  //rop_src;
    DFB_ROP2_PS_OR_ND,            //(rop_src | (~rop_dst));
    DFB_ROP2_PD_OR_PS,            //(rop_dst | rop_src);
    DFB_ROP2_ONE,                 //0xFFFF
    DFB_ROP2_NONE = 0xFF          // disable ROP mode
} DFB_GFX_ROP2_OP;
...
/* DIET_AXISMOTION */
     ...
     DFBInputDeviceID                ex_device_id;  /* source of event */
} DFBInputEvent;
...
typedef struct {
     ...
     DFBWindowDeviceID               device_id;  /* source of event */
} DFBWindowEvent;
...
/*#ifdef GFX_IOMMU_IOVA_SUPPORT remove for seperate control*/
typedef enum
{
    DFB_MTIOMMU_DMA_BIDIRECTION = 0,
    DFB_MTIOMMU_DMA_TO_DEVICE   = 1,
    DFB_MTIOMMU_DMA_FROM_DEVICE = 2,
    DFB_MTIOMMU_DMA_NONE = 3
} DFB_MTIOMMU_DMA_DIRECTION_T;
/*#endif*/


typedef struct _DFB_IOMMU_PIN_RANGE_T
{
    unsigned long long start;  /*GFX_IOMMU_IOVA_SUPPORT fix me*/
    unsigned long size;
    union{
        unsigned long pageArray;
        unsigned long long pading1;
    };
    unsigned long nrPages;
/*#ifdef GFX_IOMMU_IOVA_SUPPORT need to be kept. remove for seperate control*/
    unsigned int devIdx; /* 0: gfx, 1:gcpu, 2: imgrz */
    unsigned int pgd;
    unsigned int iova;
    DFB_MTIOMMU_DMA_DIRECTION_T direction;
    union{
        void *buf_priv; /* used to store driver-information: struct mtk_iommu_buf */
        unsigned long long pading0;
    };
/*#endif*/
} DFB_IOMMU_PIN_RANGE_T;
...
typedef enum
{
    DFB_IOMMU_DMA_BIDIRECTION = 0,
    DFB_IOMMU_DMA_TO_DEVICE   = 1,
    DFB_IOMMU_DMA_FROM_DEVICE = 2
} IOMMU_DMA_DIRECTION_T;

typedef struct _DFB_IOMMU_DMA_RANGE_T
{
    unsigned long start;
    unsigned long size;
    IOMMU_DMA_DIRECTION_T direction;
    unsigned long dma_addr;
    unsigned long pageArray;
} DFB_IOMMU_DMA_RANGE_T;


#ifdef CC_GFX_MMU
#define DFB_SYSTEMONLY                              1
#else
#define DFB_SYSTEMONLY                              0
#endif

#define DFB_DL_LOG                                  (0x0)
...
#define DSCAPS_VIDEO   (DSCAPS_FROM_FBM|DSCAPS_FROM_JPEG_VDP|      \
                        DSCAPS_3D_CROP_OFFSET|DSCAPS_PRIMARY|      \
                        DSCAPS_FOUCE_VIDEOONLY|DSCAPS_FROM_3DMM |  \
                        DSCAPS_FROM_MPEG)

#define DSCAPS_FROM_VIDEO(x)    (x&DSCAPS_VIDEO)
 

#define SURFACE_TYPE_VIDEO      (CSTF_LAYER|CSTF_CURSOR)

#define DFB_ACCESS_MMU(x)     (x&CSAF_MMU)

//extern int system_mmu_pin_mem( DFB_IOMMU_PIN_RANGE_T *pt_page );
//extern int system_mmu_unpin_mem( DFB_IOMMU_PIN_RANGE_T *pt_page );
//extern DFBResult mt53EngineSync( void *drv, void *dev );

extern int system_mmu_pin_iomem( DFB_IOMMU_PIN_RANGE_T *pt_page );
/*#define DFB_IOMMU_PRECHECK*/
#ifdef DFB_IOMMU_PRECHECK
extern unsigned int CalcPhysicalAddress(unsigned int VA, unsigned int TTB);
#endif
#define FBIO_AP_MOD_VUDU        0x1

//extern int mt53_support_mmu(void);
/*extern long long mdfb_clock_get_micros();*/


#define DFB_LOG_LVL_0         (0)
#define DFB_LOG_LVL_1         (1)
#define DFB_LOG_LVL_2         (2)
#define DFB_LOG_LVL_3         (3)
#define DFB_LOG_LVL_8         (20)
#define DFB_LOG_LVL_BLT       (10)
#define DFB_LOG_LVL_SM        (10)
#define DFB_LOG_LVL_CLCK      (10)
#define DFB_LOG_LVL_MAX       (0xff)



#ifdef CC_DFB_LOG_LVL
#define MTDFB_LOG(lvl,x...)   if(CC_DFB_LOG_LVL>=lvl){ direct_log_printf( NULL, "(MD)" x );}
#else   
#define MTDFB_LOG(l,x...)
#endif

#define DIRECT_PTR_MIN         (3000)
#define DIRECT_PTR_DGS         (10000)
#define DIRECT_CUR_MICS        mdfb_clock_get_micros()

#define DIRECT_INTERFACE_DBG_DELTA  (0x0) 

#if DIRECT_INTERFACE_DBG_DELTA 
#define DIRECT_INTERFACE_DBG_DELTA_START()                                      \
    long long         u8_start =0x0;                                            \
    long long         u8_end   =0x0;                                            \
    if(dfb_interface_dbg_delta_enable())                                        \
    {                                                                           \
       u8_start = DIRECT_CUR_MICS;                                              \
       if(!u8_start)                                                            \
       {                                                                        \
           u8_start = DIRECT_CUR_MICS;                                          \
       }                                                                        \
...
#define DIRECT_INTERFACE_DBG_DELTA_END(IDX)                                     \
     if(dfb_interface_dbg_delta_enable())                                       \
     {                                                                          \
        u8_end = DIRECT_CUR_MICS;                                               \
        dfb_interface_dbg_set_delta_data(IDX,(((u8_end>u8_start)&&(u8_end)&&(u8_start))?(u8_end- u8_start):0));\
     }
...
#define DIRECT_INTERFACE_SET_PRT_DATA() \
    static long long u8_sum=0x0;       \
    static long long u8_cnt=0x0;       \
    long long u8_st=0x0;               \
    long long u8_dif=0x0;              \
    long long u8_ed=0x0;               \
    long long u8_dlt=0x0;              \
    u8_st  = DIRECT_CUR_MICS;          \
    u8_dif = DIRECT_CUR_MICS;          \
    u8_ed  = u8_st;                    \

  
#define DIRECT_INTERFACE_CHECK_PRT_DATA()   \
    u8_dlt = (DIRECT_CUR_MICS - u8_ed);         \
    u8_ed  = DIRECT_CUR_MICS;                   \
    u8_dif = (u8_sum/u8_cnt);                   \
    if(u8_dlt>DIRECT_PTR_MIN)                   \
    {                                           \
        MTDFB_LOG(DFB_LOG_LVL_BLT,              \
        "%s[line=%d][dlt=%lld,sum=%lld,st=%lld,u8_dif=%lld,ed=%lld,u8_cnt=%lld]\n",\
        __FUNCTION__,__LINE__,u8_dlt,u8_sum,u8_st,(u8_ed-u8_st),u8_ed,u8_cnt);     \
        if(u8_dlt>DIRECT_PTR_DGS)                                            \
        {                                                                    \
            MTDFB_LOG(DFB_LOG_LVL_BLT,                                       \
            "%s[dangerous]\n",                                               \
            __FUNCTION__);                                                   \
...
#define DIRECT_INTERFACE_PRT_DATA()             \
    u8_ed  = DIRECT_CUR_MICS;                      \
    u8_sum += (u8_ed-u8_st);                       \
    u8_cnt++;                                      \
    u8_dif = (u8_sum/u8_cnt);                      \
    if((u8_ed-u8_st)>DIRECT_PTR_MIN)               \
    {                                              \
        MTDFB_LOG(DFB_LOG_LVL_BLT,                 \
        "%s[line=%d][avl=%lld,sum=%lld,st=%lld,u8_dif=%lld,ed=%lld,u8_cnt=%lld]\n",\
        __FUNCTION__,__LINE__,u8_dif,u8_sum,u8_st,(u8_ed-u8_st),u8_ed,u8_cnt);     \
        if((u8_ed-u8_st)>DIRECT_PTR_DGS)                                     \
        {                                                                    \
            MTDFB_LOG(DFB_LOG_LVL_BLT,                                       \
            "%s[dangerous]\n",                                               \
            __FUNCTION__);                                                   \
...
extern unsigned int mt_get_dbg_info( unsigned int u4_type);
#if 0//(ENABLE_DEBUG)  
#ifndef CC_DFB_DEBUG_SUPPORT
/* #define CC_DFB_DEBUG_SUPPORT (1)  */
#endif
#endif

#define CC_DFB_DEBUG_SW_ONLY                        (0x1)
#define CC_DFB_DEBUG_CARD_FILL                      (0x2)
#define CC_DFB_DEBUG_CARD_BLIT                      (0x4)
#define CC_DFB_DEBUG_CARD_SBLIT                     (0x8)
#define CC_DFB_DEBUG_CARD_CHECK                     (0x10)
#define CC_DFB_DEBUG_SURFACE                        (0x20)

#define CC_DFB_DEBUG_ALL                            (0xFFFFFFFF)

#define CC_DFB_DEBUG_FG                             (0x0)


extern void dfb_set_cursor_border(int i4_enable,int i4_x_left,int i4_x_right,int i4_y_left,int i4_y_right);
extern int dfb_get_cursor_clip_x(int i4_x);
extern int dfb_get_cursor_clip_y(int i4_x);

extern int dfb_config_get_layer_stacking(int i4_layer);

extern void* system_get_crsr_addr(void);
extern unsigned int system_get_crsr_phy(void);
extern unsigned int system_get_crsr_size(void);
extern DFBResult mdfb_wait_for_sync(void);
extern int dfb_gfxcard_get_num(void);

//extern void mt53_fill_rect(unsigned int u4_addr,unsigned int u4_pitch,unsigned int u4_h, int fgPhyEn);

extern void dfb_primary_enable_layer(unsigned int u4_layer,unsigned int u4_enable);

DFBResult IDirectFBEventBuffer_SetWindowMask( IDirectFBEventBuffer *thiz, DFBWindowEventType mask);
extern DFBResult IDirectFBDisplayLayer_SetSurface( IDirectFBDisplayLayer  *thiz, IDirectFBSurface *surface_interface);

extern void mt_set_render_option(unsigned int u4_val);
extern unsigned int mt_get_render_option(void);
//void mt53_gfx_engine_sync(void);

extern void dfb_dbg_delta_clear(void);
extern void dfb_dbg_delta_print(void);
extern u32  dfb_interface_dbg_delta_enable(void);
extern void dfb_interface_dbg_set_delta_enable(unsigned int u4_enable);
extern void dfb_interface_dbg_set_delta_data(unsigned int u4_idx,long long u8_val);

extern void dfb_gfxcard_flush_read_cache(unsigned int u4_addr, unsigned int u4_size);
extern void dfb_gfxcard_flush_texture_cache(unsigned int u4_addr, unsigned int u4_size);
extern void dfb_gfxcard_set_cache_fg(unsigned int u4_fg);
extern unsigned int dfb_gfxcard_get_cache_fg(void);
extern int system_get_fb(void);
//extern void mt53FlushInvalidCache(unsigned int u4_addr, unsigned int u4_size );
...
#define DFB_INPUT_FILER_EN
#ifdef DFB_INPUT_FILER_EN
typedef bool (*DFBInputFilter)(const DFBInputEvent *event);

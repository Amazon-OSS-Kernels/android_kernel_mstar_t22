...
_mali_osk_errcode_t mali_mem_os_put_page(struct page *page)
{
	...
	if (1 == page_count(page)) {
		atomic_sub(1, &mali_mem_os_allocator.allocated_pages);
#if defined(MSTAR)
		dma_unmap_page(&mali_platform_device->dev, _mali_page_dma_addr(page),
...
#if defined(MSTAR)
		_mali_page_set_dma_addr(new_page, dma_addr);
#else
...
#if defined(MSTAR)
			dma_unmap_page(&mali_platform_device->dev, _mali_page_dma_addr(new_page),
				       _MALI_OSK_MALI_PAGE_SIZE, DMA_BIDIRECTIONAL);
#else
...
#if defined(MSTAR)
				dma_addr_t phys = _mali_page_dma_addr(m_page->page);
#else
...
static _mali_osk_errcode_t mali_mem_os_pre_alloc_table_page(void)
{
	...
	for (i = 0; i < MALI_MEM_OS_PAGE_TABLE_PAGE_POOL_SIZE; i++)
	{
		...
		if (NULL != mapping) {
			 ...
			 * wider than 32-bit. */
#ifndef MSTAR
			/* NOTE: To support 64-bit memory address mapping,
			 * the "physical" address is allowed to be wider than 32-bit in page pool */

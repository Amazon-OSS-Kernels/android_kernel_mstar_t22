...
#if defined(MSTAR) && (LINUX_VERSION_CODE < KERNEL_VERSION(4, 10, 0))
/* Mstar: include for MSTAR_SYNC_FILE_VERSION */
...
void mali_internal_sync_timeline_signal(struct mali_internal_sync_timeline *sync_timeline)
{
	...
	list_for_each_entry_safe(sync_pt, next, &sync_timeline->sync_pt_list_head,
		...
		if ((sync_pt->base).ops->signaled &&
			(sync_pt->base).ops->signaled(&sync_pt->base)) {
...
struct mali_internal_sync_fence *mali_internal_sync_fence_fdget(int fd)
{
	struct file *file;
/* ARM patch begin */
/* check if the fd is valid fence fd, (see CAS-123733-B4B7T2).*/
#if (LINUX_VERSION_CODE < KERNEL_VERSION(4, 10, 0))
...
#endif
{
	...
	if (0 > ret)
	 ...
	 (defined(MSTAR) && defined(MSTAR_SYNC_FILE_VERSION) && \
	  (LINUX_VERSION_CODE < KERNEL_VERSION(4, 10, 0)) && \
	  (MSTAR_SYNC_FILE_VERSION >= KERNEL_VERSION(4, 9, 68)))
		/* MSTAR patch end */
		fence->error = ret;
#else
		fence->status = ret;

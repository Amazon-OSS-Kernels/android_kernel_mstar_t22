...
#if defined(MSTAR)
static inline void _mali_page_set_dma_addr(struct page *p, dma_addr_t dma_addr)
{
	SetPagePrivate(p);
	if (sizeof(dma_addr_t) > sizeof(p->private)) {
		...
		MALI_DEBUG_ASSERT(!(dma_addr & (PAGE_SIZE - 1)));
...
static inline dma_addr_t _mali_page_dma_addr(struct page *p)
{
	if (sizeof(dma_addr_t) > sizeof(p->private))

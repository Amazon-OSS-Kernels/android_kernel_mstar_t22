...
/**
 * @MSTAR RIU ADDRESS TYPE = 32 (if MSTAR_RIU_ADDRESS_TYPE_16 not defined)
 *
 * when original gpu internal register offset bigger than 0xffff it meants gpu have more than one group
 * (currently only mali 450, for example mali 450 PP broadcast internal register offset 0x016000 is group two),
 * we need left shift bit [31:16] of gpu internal register offset by 1 to fit the MSTAR RIU rule
 * (for example mali 450 PP broadcast offset 0x016000 become 0x026000)
 *
 * @MSTAR RIU ADDRESS TYPE = 16 (if MSTAR_RIU_ADDRESS_TYPE_16 is defined)
 *
 * No additional modification is needed because we already double the gpu internal register offset
 */

#ifdef MSTAR_RIU_ENABLED
#ifdef MSTAR_RIU_ADDRESS_TYPE_16

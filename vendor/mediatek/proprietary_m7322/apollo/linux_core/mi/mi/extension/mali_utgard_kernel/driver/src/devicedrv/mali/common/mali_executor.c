		...
		mali_bool in_upper_half)
{
	...
	if (in_upper_half) {
		...
		/* MSTAR patch begin*/
		/* Getting page directory table begin addr and fault phys addr*/
		struct mali_session_data* session = NULL;
		struct mali_gp_core *gp_core = mali_group_get_gp_core(group);
		if (NULL != gp_core) {
			struct mali_gp_job *job = group->gp_running_job;
			if (job) {
				session = job->session;
			}
		}
		else {
			struct mali_pp_job *job = group->pp_running_job;
			if (job) {
				session = job->session;
			}
		}
		{
			...
			u32 dte_entry = mali_hw_core_register_read(&group->mmu->hw_core, MALI_MMU_REGISTER_DTE_ADDR);
			u32 phys_addr = _mali_osk_mem_ioread32(pagedir->page_entries_mapped[MALI_MMU_PDE_ENTRY(fault_address)],  MALI_MMU_PTE_ENTRY(fault_address) * sizeof(u32));

			MALI_DEBUG_PRINT(2, ("Executor: Mali page fault detected at 0x%x (phys=0x%x) from bus id %d of type %s on %s\n",
				     (void *)(uintptr_t)fault_address,
				     phys_addr,
				     (status >> 6) & 0x1F,
				     (status & 32) ? "write" : "read",
				     group->mmu->hw_core.description));
			MALI_DEBUG_PRINT(3, ("Executor: MMU_DTE_ADDR is 0x%x, MMU rawstat = 0x%08X, MMU status = 0x%08X\n",
				     dte_entry, mali_mmu_get_rawstat(group->mmu), status));
		}
		/* MSTAR patch end*/

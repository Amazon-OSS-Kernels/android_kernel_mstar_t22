...
#if defined(MSTAR_CLOCK_DEBUG)
#include <platform/mstar/mstar_platform.h>
#endif
...
static ssize_t gpu_clock_write(struct file *filp, const char __user *ubuf, size_t cnt, loff_t *ppos)
{
	...
	{
		adjust_frequency(val);
	}
	return cnt;
};

static const struct file_operations mali_gpu_clock_fops = {
	.read  = gpu_clock_read,
	.write = gpu_clock_write,
};

static ssize_t mali_gpu_debug_read(struct file *filp, char __user *ubuf, size_t cnt, loff_t *ppos)
{
	char buf[64];
	size_t r;
	r = snprintf(buf, 64, "%u\n", mali_debug_property);
...
static ssize_t mali_gpu_debug_write(struct file *filp, const char __user *ubuf, size_t cnt, loff_t *ppos)
{
	int ret;
	char buffer[32];
	unsigned long val = 0;
	...
	if (val == mali_debug_property_password || val == 0)
	{
		mali_debug_property = val;
	}
	return cnt;
};

static const struct file_operations mali_gpu_debug_fops = {
	.read  = mali_gpu_debug_read,
	.write = mali_gpu_debug_write,
};
...
#if defined(MSTAR_CLOCK_DEBUG)
			debugfs_create_file("gpu_clock", 0644, mali_debugfs_dir, NULL, &mali_gpu_clock_fops);
			debugfs_create_file("mali_debug_level", 0644, mali_debugfs_dir, NULL, &mali_gpu_debug_fops);
#endif

...
static void mstar_phy_tuning(uintptr_t pU3phyDtopBase, uintptr_t pU3phyAtopBase,
			uintptr_t pUtmissBase, uintptr_t pUpllBase)
{
	...
	writeb(UTMISS_REF_CLK_EN_2F, (void *) (pUtmissBase + 0x2F * 2 - 1));     /*  [30] & [25] to enable PCIE REF clock */

	/* overwrite EQ @PCIE mode in FT/verification/normal function */
	writeb(ATOP_EQ_A1, (void *)(pU3phyAtopBase + 0xA1 * 2 - 1));/* TEST_RXPLL[47]=1 for turning on PD demux */
...
uint32_t mstar_pcie_enable_ppc(struct mstar_pcie_port *port)
{
	...
	u8  value, low_active;
		...
		addr_w = pcie_riu_readw(port, (offset + 0));
		addr = (uintptr_t)addr_w << 8;
		addr_w = pcie_riu_readw(port, (offset + 1));
		addr |= addr_w & 0xFF;
...
static int mstar_pcie_init_hw(struct mstar_pcie_port *port)
{
	...
	Mstar_U3phy_MCL16_init((uintptr_t)port->phyDTOP_base, (uintptr_t)port->phyATOP_base, \
							(uintptr_t)port->utmiss_base, (uintptr_t)port->upll_base);
	mstar_phy_tuning((uintptr_t)port->phyDTOP_base, (uintptr_t)port->phyATOP_base, \
							(uintptr_t)port->utmiss_base, (uintptr_t)port->upll_base);
#else

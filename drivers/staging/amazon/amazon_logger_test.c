 ...
 *
 * Copyright 2016-2019 Amazon Technologies, Inc. All Rights Reserved.
 *
...
static int metrics_test(void)
{
	...
	snprintf(metrics_log, METRICS_STR_LEN,
		 "%s:%s:100:VALUE_A=%d;IN;1,VALUE_B=%d;IN;1:NR",
		 KERNEL_METRIC_TEST_GROUP_ID,
		 KERNEL_METRICS_TEST_SCHEMA_ID, 1, 0);
#else
	snprintf(metrics_log, METRICS_STR_LEN,
		 "VALUE_A=%d;IN;1,VALUE_B=%d;IN;1:NR", 1, 0);
#endif
...
#ifdef CONFIG_AMAZON_MINERVA_METRICS_LOG
	snprintf(metrics_log, METRICS_STR_LEN,
		 "%s:%s:100:V A L U E A=%d;IN;1,V A L U E B=%d;IN;1:NR"
		 KERNEL_METRIC_TEST_GROUP_ID,
		 KERNEL_METRICS_TEST_SPACE_SCHEMA_ID, 1, 0);
#else
	snprintf(metrics_log, METRICS_STR_LEN,
		 "V A L U E A=%d;IN;1,V A L U E B=%d;IN;1:NR", 1, 0);
#endif
...
#define TIMER_TEST_SID KERNEL_METRICS_VITALS_TIMER_TEST_SCHEMA_ID
#define COUNTER_TEST_SID KERNEL_METRICS_VITALS_COUNTER_TEST_SCHEMA_ID
static int vital_test(void)
{
...
#ifdef CONFIG_AMAZON_MINERVA_METRICS_LOG
	/* key=NULL case */
		ret = log_timer_to_vitals_v2(ANDROID_LOG_INFO,
					     KERNEL_METRIC_TEST_GROUP_ID,
					     TIMER_TEST_SID,
					     "Kernel timer", "TEST",
					     "time_in_test", NULL, now.tv_sec,
					     NULL, VITALS_TIME_BUCKET,
					     NULL, NULL);
		if (ret)
...
#else
		/* key=NULL case */
		ret = log_timer_to_vitals(ANDROID_LOG_INFO,
					  "Kernel timer", "TEST",
					  "time_in_test", NULL, now.tv_sec,
					  NULL, VITALS_TIME_BUCKET);
		if (ret)
		pr_err("log_timer_to_vitals: fail key=NULL case %d\n", ret);
		/* normal case */
		ret = log_timer_to_vitals(ANDROID_LOG_INFO,
					  "Kernel timer", "TEST",
					  "time_in_test", "Test", now.tv_sec,
					  "s", VITALS_TIME_BUCKET);
		if (ret)
		pr_err("log_timer_to_vitals: fail normal case %d\n", ret);
	...
	for (i = 0; i < loop; i++) {
#ifdef CONFIG_AMAZON_MINERVA_METRICS_LOG
		/* nomral case */
		ret = log_counter_to_vitals_v2(ANDROID_LOG_INFO,
					       KERNEL_METRIC_TEST_GROUP_ID,
					       COUNTER_TEST_SID,
					       "Kernel vitals", "TEST",
					       "VITALS", "test_value",
		...
		ret = log_counter_to_vitals_v2(ANDROID_LOG_INFO,
					       ...
					       COUNTER_TEST_SID,
					       "Kernel vitals", "TEST",
					       "VITALS", NULL, (u32)1, "count",
					       NULL, VITALS_NORMAL, NULL, NULL);
		if (ret)
		pr_err("log_counter_to_vitals: fail key=metadata=NULL %d\n",
		       ret);
#else
		/* nomral case */
		ret = log_counter_to_vitals(ANDROID_LOG_INFO, "Kernel vitals",
					    "TEST", "VITALS", "test_value",
					    (u32)1, "count", "s",
					    VITALS_NORMAL);
		if (ret)
		pr_err("log_counter_to_vitals fail nomral case %d\n", ret);
		/* metadata=NULL case */
		ret = log_counter_to_vitals(ANDROID_LOG_INFO, "Kernel vitals",
					    "TEST", "VITALS", "test_value",
					    (u32)1, "count", NULL,
					    VITALS_NORMAL);
		if (ret)
		pr_err("log_counter_to_vitals: fail metadata=NULL %d\n", ret);
		/* key=NULL case */
		ret = log_counter_to_vitals_v2(ANDROID_LOG_INFO,
					       "Kernel vitals",
					       "TEST", "VITALS", NULL, (u32)1,
					       "count", "s", VITALS_NORMAL);
		if (ret)
		pr_err("log_counter_to_vitals: fail key=NULL case %d\n", ret);
		/* key=NULL, metadata=NULL case */
		ret = log_counter_to_vitals_v2(ANDROID_LOG_INFO,
					       "Kernel vitals", "TEST",
					       "VITALS", NULL, (u32)1,
					       "count", NULL, VITALS_NORMAL);
		if (ret)
		pr_err("log_counter_to_vitals: fail key=madata=NULL %d\n", ret);
#endif

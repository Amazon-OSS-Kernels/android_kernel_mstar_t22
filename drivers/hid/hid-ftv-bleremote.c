...
#define BT_DEVICE_HID_SCHEMA_ID   "b9p5/2/04334000"
	...
	u8 *data, int size)
{
	...
	case SEARCH_KEY:
		if (bleremote_dev.voice_active == true)
			...
			dbg_hid("ftvremote: ftv_remote_raw_event voice active:
				TRUE device %p\n",
				bleremote_dev.hdev);
			mutex_lock(&audio_rw_lock);

#if (KERNEL_VERSION(3, 15, 0) <= LINUX_VERSION_CODE)
			ret = hid_hw_output_report(bleremote_dev.hdev,
				     audio_start, sizeof(audio_start));
#else
			ret = bleremote_dev.hdev->hid_output_raw_report(
					bleremote_dev.hdev,
					audio_start, sizeof(audio_start),
					HID_OUTPUT_REPORT);
#endif
			if (ret < 0)
				dbg_hid("ftvremote:AudioOutput report fail\n");
			else
...
#endif
			if (ret < 0)
				dbg_hid("ftvremote:Audio output report fail\n");
			else
				bleremote_dev.audio_state_started = false;

			mutex_unlock(&audio_rw_lock);

			dbg_hid("ftvremote: voice active: FALSE device %p\n",
				bleremote_dev.hdev);
